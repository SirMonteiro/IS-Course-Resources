...
{
  // Insertion Sort ordering by PontosGanhos and SaldoDeGols in descending order
  int j;
  time key;
  for (int i = 1; i < notimes; i++)
  {
    key = timescampeonato[i];
    for (j = i - 1; j >= 0 && timescampeonato[j].PontosGanhos <= key.PontosGanhos; j--)
    {
      if (timescampeonato[j].PontosGanhos < key.PontosGanhos)
        timescampeonato[j + 1] = timescampeonato[j];
      else if (timescampeonato[j].SaldoDeGols < key.SaldoDeGols)
        timescampeonato[j + 1] = timescampeonato[j];
      else
        break;
    }
