...
/* avoid using math.h, so
 * https://www.geeksforgeeks.org/program-to-compute-log-n/
 * Recursive function that receives a number and returns the base 2 log of it
 */
...
int main(int argc, char *argv[])
{
  ...
  if (argc != 3)
  {
    printf("Este programa exige dois argumentos: <número base 2: 0 a 1024> e <arquivo .txt>\n");
    exit(1);
  }

  // read first argment and sanitize it
  int matrixSize = atoi(argv[1]);
  if ((matrixSize & (matrixSize - 1)) != 0 || matrixSize > 1024 || matrixSize == 0)
  {
    printf("O tamanho da matriz deve ser da ordem de 2^0 a 2^10\n");
    exit(1);
  }
  // calculate the exponent of matrix size
  int exponent = Log2n(matrixSize);
