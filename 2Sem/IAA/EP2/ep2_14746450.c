...
// função que dado um vetor de structs e o tamanho do vetor, ordena de forma
// crescente da chave length
void insertionSort(UPDATE *update, int n)
{
  int i, key, j;
  for (i = 1; i < n; i++)
  {
...
// função que dado um array de structs, o tamanho e o indice, faz um heap mínimo
...
// funçao que dado um array de structs e o tamanho, constrói um heap mínimo completo
void minHeapBuild(UPDATE *update, int n)
{
  for (int i = n / 2 - 1; i >= 0; i--)
    minHeapify(update, n, i);
}

// função que dado um array de structs e o tamanho respeitando a propriedade de
// heap mínimo, retorna o menor valor extraindo e refazendo a propriedade
UPDATE minHeapExtract(UPDATE **update, int *n)
{
  if (*n < 1)
  {
    ...
    error.index = -1;
    error.length = -1;
    printf("error in minHeapExtract function\n");
    return error;
  }
...
// função que dado um array de structs e o tamanho respeitando a propriedade de
// heap mínimo, retorna o menor valor
...
// função que dado um array de structs, o indice e a nova chave menor que a
// atual, troca e refaz o heap mímino
...
// função que dado um array de structs, o tamanho, o novo length e index da
// struct, insere no heap mínimo, mantendo a propriedade
...
void cenario1(int horas, FILE **file, int updatesLength, int *result)
{
  ...
  insertionSort(updates, updatesLength);
  ...
  // exiba o index
  printf("Número das atualizações:  ");
  for (int i = 0; i < updatesLength; i++)
  {
...
void cenario2(int horas, FILE **file, int updatesLength, int *result)
{
  ...
  minHeapBuild(updates, updatesLength);
  ...
  // libera o vetor de atualizações
  free(updates);
...
int main(int argc, char *argv[])
{
  ...
  {
    printf("O argumento <janela_de_horas> deve ser um número inteiro entre 14 e "
           "20\n");
    exit(1);
  }
